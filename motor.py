"""
Motor de calculo da ferramenta de planejamento de producao.

Contem apenas logica pura (leitura dos relatorios, sazonalidade, previsao
e validacao). Nao depende do Streamlit, o que permite testar por fora da
interface e reaproveitar em outro front-end.
"""
...
def normalizar(texto: str) -> str:
    """Remove acentos, espacos extras e caixa, para comparar nomes de coluna."""
...
def achar_coluna(df: pd.DataFrame, *pistas: str) -> str:
    """Localiza a coluna cujo nome contem todas as pistas informadas.

    O iFood muda o cabecalho dos relatorios de tempos em tempos, entao a busca
    e por trecho do nome em vez de igualdade exata.
    """
    for coluna in df.columns:
        ...
        if all(normalizar(p) in alvo for p in pistas):
            return coluna
    raise KeyError(
        f"Nao encontrei uma coluna contendo {pistas}. "
        f"Colunas disponiveis: {list(df.columns)}"
    )


def achar_aba(arquivo, *pistas: str) -> str:
    """Localiza a aba cujo nome contem as pistas informadas."""
    abas = pd.ExcelFile(arquivo).sheet_names
    for aba in abas:
        alvo = normalizar(aba)
        if all(normalizar(p) in alvo for p in pistas):
            return aba
    raise KeyError(f"Nao encontrei a aba {pistas}. Abas disponiveis: {abas}")
...
def interpretar_periodo(texto: str):
    ...
    if len(partes) != 2:
        raise ValueError(f"Periodo em formato inesperado: {texto}")
...
def contar_ocorrencias(inicio, fim) -> dict:
    """Conta quantas vezes cada dia da semana aparece dentro do periodo.

    Necessario porque um periodo de 62 dias pode ter 9 quartas e 8 quintas.
    Sem isso, o indice de sazonalidade fica distorcido.
    """
...
# --------------------------------------------------------------------------
# Leitura dos dois relatorios
# --------------------------------------------------------------------------

def ler_relatorio_cardapio(arquivo) -> pd.DataFrame:
    """Le a aba 'Itens' do relatorio de cardapio.
...
def ler_relatorio_vendas(arquivo) -> pd.DataFrame:
    """Le a aba 'Dias com mais vendas' do relatorio de vendas.

    Soma os pedidos das diferentes logisticas (entrega propria + retirada).
...
def carregar_historico():
    itens = pd.DataFrame(columns=["periodo", "inicio", "fim", "loja", "item", "quantidade"])
    dias = pd.DataFrame(columns=["periodo", "inicio", "fim", "dia", "pedidos"])

    if os.path.exists(ARQ_ITENS):
        itens = pd.read_csv(ARQ_ITENS, parse_dates=["inicio", "fim"])
        itens["inicio"] = itens["inicio"].dt.date
        itens["fim"] = itens["fim"].dt.date
    if os.path.exists(ARQ_DIAS):
        dias = pd.read_csv(ARQ_DIAS, parse_dates=["inicio", "fim"])
        dias["inicio"] = dias["inicio"].dt.date
        dias["fim"] = dias["fim"].dt.date
    return itens, dias


def salvar_historico(itens: pd.DataFrame, dias: pd.DataFrame):
    itens.to_csv(ARQ_ITENS, index=False)
    dias.to_csv(ARQ_DIAS, index=False)


def incorporar(historico: pd.DataFrame, novo: pd.DataFrame) -> pd.DataFrame:
    """Adiciona o novo envio ao historico, substituindo periodos repetidos.

    Evita que o usuario conte a mesma semana duas vezes se subir o arquivo
    de novo por engano.
    """
    if historico.empty:
        return novo.copy()
...
def calcular_sazonalidade(dias: pd.DataFrame):
    """Indice de sazonalidade de cada dia da semana.

        indice_d = (pedidos_d / ocorrencias_d) / (total_pedidos / dias_operados)

    Um indice de 1,30 na quarta significa que a quarta vende 30% acima da
    media de um dia qualquer de operacao.
    """
    ...
    tabela = pd.DataFrame({
        "Dia": operantes,
        "Pedidos no periodo": [pedidos[d] for d in operantes],
        "Ocorrencias do dia": [ocorrencias[d] for d in operantes],
        "Media de pedidos/dia": [pedidos[d] / ocorrencias[d] for d in operantes],
    })
    tabela["Indice de sazonalidade"] = tabela["Media de pedidos/dia"] / media_geral
...
def calcular_previsao(itens: pd.DataFrame, sazonalidade: pd.DataFrame,
    ...
    """Previsao por item e por dia da semana.
    ...
    for _, item in totais.iterrows():
        linha = {"Item": item["item"], "Media diaria": item["media_diaria"]}
        for _, dia in sazonalidade.iterrows():
            valor = item["media_diaria"] * dia["Indice de sazonalidade"] * (1 + margem)
...
def validar_modelo(itens: pd.DataFrame, dias: pd.DataFrame) -> pd.DataFrame:
    """Backtest: usa o penultimo envio para prever o ultimo e mede o erro.

    Reporta o MAPE (erro percentual absoluto medio) por item.
    """
    periodos = (
    ...
    soma_indices = saz_treino["Indice de sazonalidade"].sum()
    media_treino = (
